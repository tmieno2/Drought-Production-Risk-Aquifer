...
\affil[2]{Department of Mechanical, Aerospace and Civil Engineering, University of Manchester}
...
\caption*{\footnotesize{Note: The blue solid, red solid, and black dotted lines represent irrigated yield, rainfed yield, and water deficit, respectively.}}
\label{fig:deficit-yield-hist}
\end{figure}
...
\caption*{\footnotesize{Note: The map shows county-average aquifer thickness in 2016 for each county included in our corn and soybean regression analyses. Counties highlighted in gray are rainfed only, and the red polygon denotes the boundary of the High Plains Aquifer system.}}
\label{fig:sat-map}
\end{figure}
...
Our analysis evaluates the links between aquifer thickness and drought resilience, proposing a causal link between declining aquifer thickness and irrigated production during droughts. In the first section of our article, we propose that this connection is driven by impacts of aquifer thickness on well yields and, hence, farmers ability to use groundwater to fully buffer crops against water deficits during drought. 
...
\caption*{\footnotesize{Note: The solid black lines represent the estimated rainfed and irrigated per-area irrigated yields of corn in US High Plains at different values of water deficit and aquifer thickness. The gray shaded area represents the 95\% confidence interval.}}
...
\caption*{\footnotesize{Note: The solid black lines represent the estimated rainfed and irrigated per-area irrigated yields of soybean in US High Plains at different values of water deficit and aquifer thickness. The gray shaded area represents the 95\% confidence interval.}}
...
\caption*{\footnotesize{Note: The black solid lines represent average yield of corn (top panel) and soybean (bottom panel) in the US High Plains for different levels of water deficit and aquifer thickness. The gray shaded area represents the 95\% confidence interval.}}
...
\caption*{\footnotesize{Note: The solid black lines represent the share of acres that are irrigated as a function of aquifer thickness relative to a baseline aquifer thickness level of \Sexpr{sat_ref} m (black line). The gray shaded areas represent the 95\% confidence interval for this estimated relationship.}}
\label{fig:test-sat-share}
\end{figure}
...
\caption*{\footnotesize{Note: This figure presents a series of thin-plate spline basis functions evaluated at a range of $x$ values.}}
\label{fig:sp-basis}
\end{figure}
...
\caption*{\footnotesize{Note: This figure presents two example functions derived by applying different coefficients on the thin-plate spline basis functions.}}
\label{fig:ex-fx}
\end{figure}
\clearpage
...
\caption*{\footnotesize{Note: This figure compares the variations in aquifer thickness after demeaning by yearly average (top), state-year average (middle), and county-year average (bottom).}}
...
\caption*{\footnotesize{Note: The solid black lines represent the estimated share of irrigated acres for corn and soybean under different types of fixed effects. Gray shaded area represents 95\% confidence interval.}}
