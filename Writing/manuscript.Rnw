...
These data were combined to form four sets of data for regression analysis: (1) per-acre corn yield ($\Sexpr{nobs_corn}$ observations), (2) per-acre soybean yield ($\Sexpr{nobs_soy}$ observations), (3) share of irrigated corn ($\Sexpr{nobs_corn_share}$ observations), and (4) share of irrigated soybean ($\Sexpr{nobs_soy_share}$ observations).
