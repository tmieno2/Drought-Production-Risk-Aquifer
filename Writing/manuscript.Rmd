...
## Aquifer depletion increases sensitivity of crop yields to drought {#impact-intensive}
...
```{r yield-response, fig.cap = "The impact of water deficit and aquifer thickness on rainfed and irrigated per-area irrigated yields of corn and soybean in US High Plains", out.width = "6.5in", cache = FALSE}
knitr::include_graphics(here::here("GitControlled/Writing/Figures/g_yield_intensive.pdf"))
```

```{r yield-dif, out.width = "6in", fig.cap = "The difference in yield for the counties with the second and third (highest) aquifer thickness groups relative to the first (lowest) aquifer thickness group."}
...
share_high <- 
  corn_share[which.min(abs(sat - 150)), ir_share_hat] %>%
  round(digits = 2)
...
Further, notice that the impact of aquifer thickness on the decline in average per-area production is highly non-linear at high seasonal water deficits. Going from 130m aquifer thickness to 90m (decline of 40m) would result in merely `r round(res_at_950[sat == 130 & crop == "corn", avg_yield] - res_at_950[sat == 90 & crop == "corn", avg_yield], digits = 2)` tonne/ha of production loss for corn. This negligible difference in average yield from the decline is due to the insensitivity of the share of irrigated acres to aquifer thickness when the aquifer thickness level is high.
Going from 90m aquifer thickness to 50m (decline of 40m) would result in `r round(res_at_950[sat == 90 & crop == "corn", avg_yield] - res_at_950[sat == 50 & crop == "corn", avg_yield], digits = 2)` tonne/ha of production loss for corn. However, going from 50m aquifer thickness to 10m (decline of 40m) would result in `r round(res_at_950[sat == 50 & crop == "corn", avg_yield] - res_at_950[sat == 10 & crop == "corn", avg_yield], digits = 2)` tonne/ha of production loss for corn. That is, the same degree of decline is much more damaging when the decline is from a low aquifer thickness level than a higher aquifer thickness level. This is primarily due to the non-linear impact of aquifer thickness on the share of irrigated corn acres as shown in Figure \@ref(fig:ir-share). When aquifer thickness level is high enough (e.g., 100m), a small decline does not reduce the share of irrigated acres. However, when aquifer thickness level is low (e.g., 40m), a small decline does reduce the share of irrigated acres. 

```{r tot-impact, fig.cap = "Average productivity of corn and soybean in the US High Plains for different levels of water deficit and aquifer thickness", out.width = "6in"}
knitr::include_graphics(here::here("GitControlled/Writing/Figures/g_total_impact.pdf"))
```

```{r dif-tot-impact, fig.cap = "Difference in average yield relative to the 1st aquifer thickness quantile for corn and soybean", out.width = "6in", cache = FALSE}
...
```{r sat-map, fig.cap = "Map showing aquifer thickness in 2016 for each county included in our corn and soybean regression analyses. Counties highlighted in grey are rainfed only, and the red polygon denotes the boundary of the High Plains Aquifer system", out.width = "6in"}
...
```{=Tex}
\begin{equation}
y_{i,t} = \sum_{j=1}^4 \sigma_j C_j + \sum_{j=1}^4 \sum_{k}^K \beta_{j,k}\phi_{j,k}(WB_{i,t})\cdot C_j  + \alpha_i + \phi_t + v_{i,t} (\#eq:eqn1)
\end{equation}
```
...
```{r irrigated-yield-ind-corn, fig.cap = "The impact of water deficit and aquifer thickness on rainfed and irrigated per-area irrigated yields of corn in US High Plains with 95 percent confidence interval", out.width = "6in", cache = FALSE}
knitr::include_graphics(here::here("GitControlled/Writing/Figures/g_yield_with_conf_corn.pdf"))
```

```{r irrigated-yield-ind-soy, fig.cap = "The impact of water deficit and aquifer thickness on rainfed and irrigated per-area irrigated yields of soybean in US High Plains with 95 percent confidence interval", out.width = "6in", cache = FALSE}
knitr::include_graphics(here::here("GitControlled/Writing/Figures/g_yield_with_conf_soy.pdf"))
```

```{r avg-yield-ind, fig.cap = "Average productivity of corn and soybean in the US High Plains for different levels of water deficit and aquifer thickness (The shaded areas represent the 95 percent confidence interval)", out.width = "6.5in", cache = FALSE}
