...
authors:
- name: Taro Mieno
  address: Department of Agricultural Economics, University of Nebraska Lincoln
  email: tmieno2@unl.edu
  footnote: Something here
- name: Timothy Foster
  address: Department of WWW, University of Manchester
...
#| cache: false
knitr::purl(
  here::here("Codes/1_analysis.Rmd"),
  here::here("Codes/1_analysis.R"),
)
source(here::here("Codes/1_analysis.R"))

knitr::purl(
  here::here("Codes/2_figures_tables.Rmd"),
  here::here("Codes/2_figures_tables.R"),
)
source(here::here("Codes/2_figures_tables.R"))

knitr::purl(
  here::here("Codes/3_appendix.Rmd"),
  here::here("Codes/3_appendix.R"),
)
source(here::here("Codes/3_appendix.R"))
```
...
@fig-geo-focus shows the geographical coverage of the data for regression analysis for corn and soybean. 
...
Regression results are presented graphically rather than in tables because individual coefficients estimates on spline basis functions cannot be readily interpreted and also because visualized results are much easier to interpret. Figure @fig-yield shows yield response to water deficit for dryland production and groundwater-irrigated production at different levels of saturated thickness categories for corn (1st row) and soybean (2nd row).  
...
In a fairly wet year without much water stress, dryland and irrigated production result in very similar crop yields for both soybean and corn yields. Irrigated yields are only slighly higher than dryland yields at a negative value of water deficit (precipitation is greater than reference evapotraspiration). However, dryland yield declines sharply and steadily as water deficit increases. On the contrary, irrigated corn yield actually increase as water deficit increases up to about water deficit of 400mm at all the saturated thickness levels for both corn and soybean. This is because irrigation can turn unfavorable high-heat conditions into favorable growing conditions up to a point (Tim, citations?). However, yield response to water deficit departs after deficit of about 400mm depending on the level of saturated thickness. While higher saturated thickness categories can withstand water deficit and crop yield stays about the same for higer watger deficit values, the low saturated thickness category experience non-trivial declines in crop yields as water deficit increases. 
...
Some studies use SPEI to represent drought severity. We also considered using SPEI, but the regressions with water deficit provides a better fit for both corn and soybean (see @tbl-fit). 
...
data.table(
  ...
  `R-squared` = c(r2_corn_wb, r2_corn_spei, r2_corn_wb, r2_soy_spei)
) %>%
modelsummary::datasummary(`R-squared` ~ Crop * Variable * (Mean), data = .)
```

Qualitatively, the results with SPEI are similar to the regression reulsts with water deficit as can be seen in @fig-spei.
...
<!-- ## Continuous saturated thickness -->
