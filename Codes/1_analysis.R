...
# semi_formula <-
#   formula(yield ~ s(balance, k = 3, m = 2) + s(sat, k = 3, m = 2) + ti(sat, balance, k = 3, m = 2))
...
#   semi_ols(
#     semi_formula = semi_formula,
#     c_formula = "i(year) + i(sc_code)",
#     # fe = "",
#     cluster = "sc_code",
#     data = data_corn[sat_cat != "dryland", ]
#   )
...
# modelsummary::msummary(ir_share_res_corn, stars = TRUE)
...
ir_share_res_soy <-
  gam(
    acres_ratio ~
      s(sat, k = 4, m = 2) + s(balance_avg, k = 3, m = 2) +
      ti(sat, balance_avg, k = 3, m = 2) + factor(year),
    data = ir_share_data_soy,
    # weight = ~acres,
    family = binomial
  )

# modelsummary::msummary(ir_share_res_soy, stars = TRUE)
