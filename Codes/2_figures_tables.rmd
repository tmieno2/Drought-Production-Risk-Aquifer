...
```{r}
all_results <- readRDS("Shared/Results/all_results.rds")
```
...
g_sat_dist <-
  ...
  .[sat != 0, ] %>%
  ggplot(data = .) +
  geom_histogram(aes(x = sat), fill = "white", color = "blue") +
...
ggsave("GitControlled/Writing/Figures/g_yield_with_conf_corn.pdf", g_yield_with_conf[crop == "corn", g_fig][[1]], width = 6, height = 3.5)
ggsave("GitControlled/Writing/Figures/g_yield_with_conf_soy.pdf", g_yield_with_conf[crop == "soy", g_fig][[1]], width = 6, height = 3.5)
```
...
g_share <-
  ggplot(plot_data_ir_share[sat <= 150, ]) +
...
# Difference in the share of irrigated production (from 130)

```{r}
g_share_dif <-
  ggplot(plot_data_ir_share[sat < 105, ]) +
...
ggsave(
  file = "GitControlled/Writing/Figures/g_share_dif.pdf",
  g_share_dif,
  height = 4,
  width = 5
)
...
ggsave("GitControlled/Writing/Figures/g_avg_yield_dif.pdf", g_ay, width = 6, height = 5)
```
...
ggsave("GitControlled/Writing/Figures/g_avg_yield.pdf", g_ay_response, width = 6, height = 3.5)
```
