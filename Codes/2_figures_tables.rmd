...
g_ir_yield_both <-
  ...
  dplyr::mutate(plot_data = purrr::map2(yield_response_sum, crop, \(x, crop) {
    x[, sat_cat_text := case_when(
      sat_cat == "[39.4,82]" ~ "1st quantile: [12m, 25m]",
      sat_cat == "(82,194]" ~ "2nd quantile: (25m, 59.1m]",
      sat_cat == "(194,708]" ~ "3rd quantile: (59.1m, 215m]",
      sat_cat == "dryland" ~ "Rainfed"
    )] %>%
      .[, sat_cat_text := factor(
        sat_cat_text,
        levels = c("Rainfed", "1st quantile: [12m, 25m]", "2nd quantile: (25m, 59.1m]", "3rd quantile: (59.1m, 215m]")
      )] %>%
      .[, crop_type := crop] %>%
      .[, crop_type := case_when(
...
ggsave("Shared/Results/g_yield_with_conf_soy.pdf", g_yield_with_conf[crop == "soy", g_fig][[1]], width = 6, height = 3.5)
```
